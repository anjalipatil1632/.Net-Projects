...
namespace EmployeeTravelBookingSystem_WebApi.Controllers
{
    ...
    {
        EmployeeTravelBookingSystemEntities empentity = new EmployeeTravelBookingSystemEntities();
        [HttpGet]
        public IHttpActionResult gettravelrequest(string LoginID)
        {
            var results = empentity.spGetTravelRequestDetails(LoginID).ToList();
            if (results == null)
            {
        ...
        {
            var result = empentity.AddTravelRequest(travelRequest.FromLocation, travelRequest.ToLocation, travelRequest.FromDate, travelRequest.ToDate, travelRequest.Medium, travelRequest.UserId);
            if (result == null)
            {
                var response = new HttpResponseMessage(HttpStatusCode.NotFound)
                {
                    Content = new StringContent(string.Format("Ticket not raise successfully ")),
                    ReasonPhrase = "Ticket not raise successfully"
                };
